...
OUTLOOK_CATEGORIES = (
    OutlookCategory(2, "TSTM", "General Thunderstorms Risk", "#55BB55", "#C1E9C1"),
    OutlookCategory(3, "MRGL", "Marginal Risk", "#005500", "#66A366"),
    OutlookCategory(4, "SLGT", "Slight Risk", "#DDAA00", "#FFE066"),
    OutlookCategory(5, "ENH", "Enhanced Risk", "#FF6600", "#FFA366"),
    OutlookCategory(6, "MDT", "Moderate Risk", "#CC0000", "#E06666"),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a model-derived SPC-style categorical convective outlook from GFS "
            "precipitation rate, CAPE, helicity, and lifted index."
        )
    )
    parser.add_argument(
        "--output-root",
...
def make_output_dirs(output_root: Path, run_spec: RunSpec) -> tuple[Path, Path, Path, Path]:
    run_root = output_root / f"gfs_outlook_{run_spec.run_stamp}"
...
def read_outlook_fields(grib_path: Path) -> tuple[dict[str, np.ndarray], np.ndarray, np.ndarray, dt.datetime]:
    ...
    prate_message = lookup({"prate"}, ("precipitation rate",))
    cape_message = lookup({"cape"}, ("convective available potential energy",))
    hlcy_message = lookup({"hlcy"}, ("storm relative helicity",))
    lifted_message = lookup({"4lftx", "lftx"}, ("lifted index", "4-layer lifted index"))
    vvel_message = lookup_optional({"vvel", "w"}, ("vertical velocity", "pressure vertical velocity"))

    prate, lats, lons = read_field(prate_message)
    cape, _, _ = read_field(cape_message)
    hlcy, _, _ = read_field(hlcy_message)
    lifted, _, _ = read_field(lifted_message)
    if vvel_message is None:
        vvel = np.zeros_like(prate)
    else:
        vvel, _, _ = read_field(vvel_message)
    valid_time = prate_message.validDate.replace(tzinfo=dt.timezone.utc)
...
def majority_filter(mask: np.ndarray, threshold: int = 5) -> np.ndarray:
...
def derive_outlook_categories(fields: dict[str, np.ndarray]) -> np.ndarray:
    prate = fields["prate"]
    cape = fields["cape"]
    hlcy = fields["hlcy"]
    ...
    precip_mask = prate >= 0.02
    precip_seed = precip_mask & (cape >= 25.0) & (lifted <= 6.0)
    stretched_precip_seed = precip_seed.copy()
    for _ in range(2):
        stretched_precip_seed = expand_mask(stretched_precip_seed, threshold=1)
    tstm_support = smooth_field(stretched_precip_seed.astype(float), passes=7)

    score = np.zeros_like(prate, dtype=np.int16)
    score += (cape >= 150.0).astype(np.int16)
    score += (cape >= 500.0).astype(np.int16)
    score += (cape >= 1000.0).astype(np.int16)
    score += (cape >= 2000.0).astype(np.int16)
    score += (hlcy >= 50.0).astype(np.int16)
    score += (hlcy >= 150.0).astype(np.int16)
    score += (hlcy >= 250.0).astype(np.int16)
    score += (hlcy >= 400.0).astype(np.int16)
    score += (lifted <= 0.0).astype(np.int16)
    score += (lifted <= -2.0).astype(np.int16)
    score += (lifted <= -4.0).astype(np.int16)
    score += (lifted <= -6.0).astype(np.int16)
    score += (prate >= 0.05).astype(np.int16)
    score += (prate >= 0.25).astype(np.int16)
    score += (prate >= 0.75).astype(np.int16)
    score += (prate >= 1.5).astype(np.int16)
    score += (vvel <= -0.05).astype(np.int16)
    score += (vvel <= -0.1).astype(np.int16)
    score += (vvel <= -0.2).astype(np.int16)
    severity = smooth_field(score.astype(float), passes=4)
    ...
    def grouped_mask(
        ...
        mask = seed & (support_field >= min_support)
        mask = fill_mask_holes(mask)
        for _ in range(expansions):
            mask = expand_mask(mask, threshold=2)
            mask &= support_field >= min_support
        mask = majority_filter(mask, threshold=4)
        mask = fill_mask_holes(mask)
        return mask

    tstm_area = grouped_mask(
        tstm_support >= 0.14,
        min_support=0.1,
        expansions=2,
        support_field=tstm_support,
    )
    mrgl_area = grouped_mask(
        (tstm_support >= 0.24) & (severity >= 3.4),
        min_support=0.17,
        expansions=2,
        support_field=tstm_support,
    )
    slgt_area = grouped_mask(
        (tstm_support >= 0.34) & (severity >= 8.4),
        min_support=0.25,
        expansions=1,
        support_field=tstm_support,
    )
    enh_area = grouped_mask(
        (tstm_support >= 0.42) & (severity >= 12.0) & (cape >= 1600.0),
        min_support=0.31,
        expansions=1,
        support_field=tstm_support,
    )
    mdt_area = grouped_mask(
        (tstm_support >= 0.46)
        & (severity >= 14.2)
        & (cape >= 2500.0)
        & (hlcy >= 300.0)
    ...
    categories = np.zeros_like(score, dtype=np.int16)
    categories[tstm_area] = 2
    categories[mrgl_area] = 3
    categories[slgt_area] = 4
    categories[enh_area] = 5
    categories[mdt_area] = 6
    return categories
...
def format_display_time(timestamp: dt.datetime) -> str:
    ...
    hour_label = local_time.strftime("%I").lstrip("0") or "12"
    return f"{local_time.strftime('%A')} {hour_label}:{local_time.strftime('%M %p')}"
...
def render_outlook(
    ...
    axis.set_title(
        "GFS Model-Derived Convective Outlook\n"
        f"Init: {init_label}   Valid: {valid_label}   Forecast Hour: F{forecast_hour:03d}",
        fontsize=16,
        weight="bold",
        loc="left",
    )
...
def main() -> int:
    ...
    session = requests.Session()
    session.headers.update({"User-Agent": "gfs-convective-outlook/1.0"})
    ...
    for forecast_hour in FORECAST_HOURS:
        base_name = f"gfs_outlook_{run_spec.run_stamp}_f{forecast_hour:03d}"
        ...
        print(f"Building model outlook for F{forecast_hour:03d}")
        try:
            fields, lats, lons, valid_time = read_outlook_fields(grib_path)
            categories = derive_outlook_categories(fields)
            render_outlook(
                png_path=png_path,
    ...
    print("Finished generating model-derived convective outlooks.")
    return 0
